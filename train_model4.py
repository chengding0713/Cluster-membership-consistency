from resnet1d_stark import Resnet34
from dataset_stark import Dataset_per_file, Dataset_PPG, Dataset_ori,Dataset_PPG_clutser
...
LAMBDA = 0.9
subset = 0
MODEL_FOLDER = f'epoch_{NUM_EPOCHS}_ppglr_{PPG_LR}_lambda_{LAMBDA}_intra_nosoftmax'
...
def train_epoch(epoch_idx, PPG_model, ce_loss_fn, PPG_optimizer, train_loader, lambda_):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        z_0 = PPG_feature[cluster==0]
        z_1 = PPG_feature[cluster==1]
        # Zero the gradient

        intra_loss = intra_cluster_loss(z_0)+ intra_cluster_loss(z_1)
        inter_loss = inter_cluster_loss(z_0, z_1)
        # # Now backward loss
        # inter_loss_weight = intra_loss.item() / inter_loss.item()
        alpha = 0.99

        # loss = alpha * PPG_loss + (1 - alpha) * (intra_loss + inter_loss_weight * inter_loss)
        loss = alpha * PPG_loss + (1 - alpha) * intra_loss
        ...
        if batch_idx % 100 == 0:
            print_flush(
                f'\t[TRAIN] Epoch {epoch_idx} Batch {batch_idx}/{len(train_loader)} Loss: {train_loss / (batch_idx + 1)}, \tPPG F1: {PPG_f1s / (batch_idx + 1)}, \tBatch Avg-T: {(batch_tend - tstart) / (batch_idx + 1)}')
...
def eval_epoch(epoch_idx, PPG_model, ce_loss_fn, val_loader, lambda_):
    with torch.no_grad():
        ...
        tend = datetime.now()
        print_flush(
            f'[VAL] Epoch {epoch_idx} Loss: {val_loss / (batch_idx + 1)}, \tPPG F1: {f1_score(all_targets.detach().cpu().numpy(), PPG_preds.detach().cpu().numpy())}')
...
if __name__ == '__main__':
    ...
    train_dataset = Dataset_PPG_clutser('/opt/localdata/storage/stark_stuff/ppg_ecg_project/data/', split='train',
                                subset=subset)
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=0)
    val_dataset = Dataset_PPG('/opt/localdata/storage/stark_stuff/ppg_ecg_project/data/', split='val', subset=subset)
    val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)

    print_flush('Dataset finished')

    PPG_model = Resnet34().to(device)
