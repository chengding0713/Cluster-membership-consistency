...
class Dataset_whole(Dataset):
    ...
    def build_dataset(self):
        ...
        print_flush(
            f'\tloading {self.split} finished t={datetime.now() - tstart}, mem used={psutil.virtual_memory()[3] / 1000000000}')
        ...
        print(
            f'dataset built AF counts {len(AF_labels)}, NSR counts {len(NSR_labels)}, total counts {len(self.all_labels)}',
            flush=True)
...
class Dataset_PPG_clutser(Dataset):
    def __init__(self, data_path, split, subset):
        super().__init__()
        self.data_path = data_path
        self.split = split
        ...
        self.length = len(self.all_labels)
        self.subset = subset
        self.cluster = torch.from_numpy(np.load('/opt/localdata/storage/chengding_project_data/alarm_data_npy/y_cluster_2400_2.npy')).long()
    ...
    def __getitem__(self, idx):
        ...
        if idx < self.AF_PPG.shape[0]:
            return self.AF_PPG[idx][None, :], self.all_labels[idx], self.cluster[idx]
        elif idx >= self.AF_PPG.shape[0] and idx < (self.AF_PPG.shape[0] + self.PVC_PPG.shape[0]):
            offset = self.AF_PPG.shape[0]
            return self.PVC_PPG[idx - offset][None, :], self.all_labels[idx], self.cluster[idx]
        else:
            offset = self.AF_PPG.shape[0] + self.PVC_PPG.shape[0]
            return self.NSR_PPG[idx - offset][None, :], self.all_labels[idx], self.cluster[idx]
